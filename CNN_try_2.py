...
'''
m = CNN()
m.add(Conv2d(input_shape = (28, 28, 1), filters = 6, padding="same", kernel_size=(5, 5), activation="relu"))
m.add(Pool2d(kernel_size=(2, 2), stride=2))
m.add(Conv2d(filters=16, kernel_size=(5, 5), padding=None, activation="relu"))
m.add(Pool2d(kernel_size=(2, 2), stride=2))
m.add(Flatten())
m.add(FFL(neurons = 120, activation = "relu"))
m.add(FFL(neurons = 100, activation = "relu"))
m.add(Dropout(0.1))

m.add(FFL(neurons = 10, activation='softmax'))
m.compile_model(lr=0.2, opt="sgd", loss="cse")
m.summary()
m.train(x[:300], y[:300], epochs=7, batch_size=30, val_x=xt[:200], val_y=yt[:200])
m.visualize()
m.save_model()
#load_model()
#m.summary()
